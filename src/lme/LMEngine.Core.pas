...
{ llama sampling }
function  _llama_sampling_init(): Pointer; cdecl; external LMENGINE_DEPS_DLL;
function  _llama_sampling_sample(ctx_sampling: Pointer; ctx_main, ctx_cfg: Pllama_context; idx: integer = -1): llama_token; cdecl; external LMENGINE_DEPS_DLL;
procedure _llama_sampling_accept(ctx_sampling: Pointer; ctx_main: Pllama_context; id: llama_token; apply_grammar: Boolean); cdecl; external LMENGINE_DEPS_DLL;
procedure _llama_sampling_free(ctx_sampling: Pointer); cdecl; external LMENGINE_DEPS_DLL;

type
  cerr_callback = procedure(const text: PUTF8Char; user_data: Pointer); cdecl;

procedure redirect_cerr_to_callback(callback: cerr_callback; user_data: Pointer); cdecl; external LMENGINE_DEPS_DLL;
procedure restore_cerr(); cdecl; external LMENGINE_DEPS_DLL;
...
procedure TInfero_CErrCallback(const AText: PUTF8Char; AUserData: Pointer); cdecl;
begin
  if Assigned(AUserData) then
...
begin
  ...
  try
    ...
    redirect_cerr_to_callback(TInfero_CErrCallback, Self);
    ...
    FModelParams.progress_callback := TInfero_ModelLoadProgressCallback;
